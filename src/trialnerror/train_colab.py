...
# Adjust grad_accumulation_factor and num_batches and eval_every
according to save results

{
    "pretrained_weight":  "roberta-large",
    "dataset": "infotabs/drr",
    "max_text_length": 256,
    "batch_size": 1,
    "eval_batch_size": 1,
    "num_batches": 10,
...
def train(config):
    ...
    print("total_batch: ", tot_num_batches)
    ...
    for i in range(tot_num_batches):
        ...
        print("Yes")
        print("Finished %d batches" % batch_idx,", ",(batch_idx + 1) % config.eval_every," , ",(i+1) % config.grad_accumulation_factor)

        if ((batch_idx + 1) % config.eval_every == 0) and ((i+1) % config.grad_accumulation_factor == 0):

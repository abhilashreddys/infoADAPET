...
class equateReader(object):
    '''
    equateReader reads EQUATE (Evaluating Quantitative Understanding Aptitude in Textual Entailment) dataset
    Original: https://github.com/AbhilashaRavichander/EQUATE
    '''
    ...
    def _get_file(self, split):
        ...
        if split.lower() == "train":
            ...
            raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), split)
        return file
        ...
        with open(file,'r') as tsvfile:
            ...
            for i, line in enumerate(tsvreader):
                if(i == 0): # header
                    ...
                    try:
                        dict_input = {}
                        dict_input["premise"] = line[1]
                        dict_input["hypothesis"] = line[2]
                        if(self.config.cmlm):
                            dict_input["cwords"] = self.getnmask(dict_input["hypothesis"])
                        dict_input["idx"] = str(line[0])
                        dict_output = {}
                        dict_output["lbl"] = int(line[3])
                        dict_input_output = {"input": dict_input, "output": dict_output}
                        data.append(dict_input_output)
                    except:
                        cnt += 1
        if(cnt > 0):
            print("Error in reading {0} lines from {1}".format(cnt, split))
        print("Read {0} lines from {1}".format(len(data), split))
        return data
    @property
    def pets(self):
